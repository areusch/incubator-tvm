  ...
  while (reader->NextObjectItem(reader, key, sizeof(key))) {
    if (!strcmp(key, "op")) {
      ...
      tvm_crt_error_t err = TVMPlatformMemoryAllocate(
          sizeof(TVMGraphExecutorNodeEntry) * num_inputs, ctx, (void**)&node->inputs);
...
int TVMGraphExecutor_Load(TVMGraphExecutor* executor, JSONReader* reader) {
  ...
  while (reader->NextObjectItem(reader, key, sizeof(key))) {
    if (!strcmp(key, "nodes")) {
      ...
      DLContext ctx = {kDLCPU, 0};
      tvm_crt_error_t err = TVMPlatformMemoryAllocate(sizeof(TVMGraphExecutorNode) * num_items, ctx,
                                                      (void**)&executor->nodes);
      if (err != kTvmErrorNoError) {
        fprintf(stderr, "memory allocate error: %08x", err);
        status = -1;
...
int TVMGraphExecutor_LoadParams(TVMGraphExecutor* executor, const char* param_blob,
  ...
  DLContext ctx = {kDLCPU, 0};
  tvm_crt_error_t err =
      TVMPlatformMemoryAllocate(TVM_CRT_STRLEN_NAME * executor->nodes_count, ctx, (void**)&names);
  if (err != kTvmErrorNoError) {
    fprintf(stderr, "memory allocate error: %08x", err);
    status = -1;
    return status;
  ...
  for (idx = 0; idx < size; idx++) {
    ...
    if (executor->data_entry[eid].dl_tensor.shape) {
      err = TVMPlatformMemoryFree(executor->data_entry[eid].dl_tensor.shape, ctx);
      if (err != kTvmErrorNoError) {
        status = -1;
      }
    ...
    if (executor->data_entry[eid].dl_tensor.data) {
      err = TVMPlatformMemoryFree(executor->data_entry[eid].dl_tensor.data, ctx);
      if (err != kTvmErrorNoError) {
        status = -1;
      }
...
int TVMGraphExecutor_SetupStorage(TVMGraphExecutor* executor) {
  ...
  for (idx = 0; idx < attrs->shape_count; idx++) {
    ...
    int device_type = executor->ctxs[0].device_type;
  ...
  for (idx = 0; idx < pool_entry_count; idx++) {
    ...
    TVMContext ctx = executor->ctxs[0];
  ...
  err = TVMPlatformMemoryAllocate(sizeof(TVMNDArray) * executor->data_entry_count, alloc_ctx,
...
int TVMGraphExecutor_Create(const char* sym_json, TVMModuleHandle module_handle,
                            const TVMContext* ctxs, TVMGraphExecutor** executor) {
  DLContext ctx = {kDLCPU, 0};
  tvm_crt_error_t err = TVMPlatformMemoryAllocate(sizeof(TVMGraphExecutor), ctx, (void**)executor);
  if (err != kTvmErrorNoError) {
    fprintf(stderr, "memory allocate error: %08x", err);
    return -1;
  }
  ...
  // init
  return TVMGraphExecutor_Init(*executor, sym_json, module_handle, ctxs);
}
...
int TVMGraphExecutor_Release(TVMGraphExecutor** pptr) {
  ...
  DLContext ctx = {kDLCPU, 0};
  status = TVMPlatformMemoryFree(executor->nodes, ctx);
  if (status != 0) {
    return status;
  }
  ...
  for (idx = 0; idx < executor->data_entry_count; ++idx) {
    status = TVMPlatformMemoryFree(executor->data_entry[idx].dl_tensor.shape, ctx);
    if (status != 0) {
      return status;
    }
  }
  status = TVMPlatformMemoryFree(executor->input_nodes, ctx);
  if (status != 0) {
    return status;
  }
  status = TVMPlatformMemoryFree(executor->node_row_ptr, ctx);
  if (status != 0) {
    return status;
  }
  status = TVMPlatformMemoryFree(executor->outputs, ctx);
  if (status != 0) {
    return status;
  }
  status = TVMPlatformMemoryFree(executor->storage_pool, ctx);
  if (status != 0) {
    return status;
  }
  status = TVMPlatformMemoryFree(executor->data_entry, ctx);
  if (status != 0) {
    return status;
  }
  status = TVMPlatformMemoryFree(executor->op_execs, ctx);
  if (status != 0) {
    return status;
  }
